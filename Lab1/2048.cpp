...
public:
	...
	virtual float estimate(const board &b) const
	{
		...
		for (int i = 0; i < iso_last; i++)
		{
			...
			sum += operator[](idx); //return weight[i];
		}
		return sum;
	}
...
public:
	...
	void dump(const board &b, std::ostream &out = info) const
	{
		for (int i = 0; i < iso_last; i++)
		{
			...
			for (size_t k = 0; k < isomorphic[i].size(); k++)
			{
				out << std::hex << ((index >> (4 * k)) & 0x0f);
			}
...
protected:
	...
	{
		// TODO 根據 pattern把這些格子的 tile 值（0~15）打包成index一個索引值，用來查 LUT
		size_t idx = 0;
		for (size_t k = 0; k < patt.size(); k++)
		{
			int t = b.at(patt[k]) & 0x0f;  // 只留 4 bit
			idx |= t << (4 * k); // 疊到對應 nibble
		}
		return idx;
	}
...
public:
			...
			if (move->assign(b))
			{
				...
				for (int i = 0; i < 16; ++i) {
					if (tmp.at(i) == 0) {
						zero_count++;
						tmp.set(i, 1);
						expected_value += 0.8f * estimate(tmp);
						tmp.set(i, 2);
						expected_value += 0.2f * estimate(tmp);
						tmp.set(i, 0);
					}
				}

				if (zero_count > 0)
					expected_value /= zero_count;
	...
	{	//TODO
		...
		// while (!path.empty() && !path.back().is_valid()) path.pop_back();
		// if (path.empty()) return;

		// double bootstrap = 0;            
		// for(int i = (int)path.size() - 1; i >= 0; i--){
		// 	const state& st = path[i];
		// 	if (!st.is_valid()) continue;

		// 	const board& s = st.before_state();
		// 	float Vs = estimate(s);
		// 	float err = float(bootstrap) - Vs;    
		// 	update(s, alpha * err);               

		// 	float Vs_new = estimate(s);
		// 	bootstrap = st.reward() + Vs_new;     
		// }
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			info << n << "\t" "mean = " << mean << "\t" "max = " << max << std::endl;
			for (int t = 1, c = 0; c < unit; c += stat[t++]) {
				if (stat[t] == 0) continue;
				int accu = std::accumulate(stat + t, stat + 16, 0);
				info << "\t" << ((1u << t) & ~1u) << "\t" << (accu * coef) << "%";
				info << "\t(" << (stat[t] * coef) << "%)" << std::endl;
			}

			// 建議：把 static 放在函式頂部或外部；這裡保留你的做法也沒關係
			static std::ofstream log("log.csv", std::ios::out | std::ios::trunc);
			if (n == (size_t)unit) {
				log << "episode,mean,max" << std::endl; // 只寫一次表頭
			}
			log << n << "," << mean << "," << max << std::endl;
...
int main(int argc, const char *argv[])
{
	...
	// tdl.add_feature(new pattern(std::vector<int>({0, 1, 2, 3, 4, 5})));
	// tdl.add_feature(new pattern(std::vector<int>({4, 5, 6, 7, 8, 9})));
	// tdl.add_feature(new pattern(std::vector<int>({0, 1, 2, 4, 5, 6})));
	// tdl.add_feature(new pattern(std::vector<int>({4, 5, 6, 8, 9, 10})));
	// tdl.add_feature(new pattern(std::vector<int>({1, 5, 6})));
	// tdl.add_feature(new pattern(std::vector<int>({0, 1, 2, 5})));
	// tdl.add_feature(new pattern(std::vector<int>({0, 1, 2, 5, 9})));
	// tdl.add_feature(new pattern(std::vector<int>({4, 5, 0, 8, 9, 10})));
	tdl.add_feature(new pattern(std::vector<int>({0, 1, 4, 5, 8, 9})));
	tdl.add_feature(new pattern(std::vector<int>({1, 2, 5, 6, 9, 10})));
	tdl.add_feature(new pattern(std::vector<int>({2, 6, 10, 14})));
	tdl.add_feature(new pattern(std::vector<int>({3, 7, 11, 15})));
	// restore the model from file
	tdl.load("weight_28000.bin");
	// tdl.load("");
	...
	info << "Training finished in " << elapsed.count() << " seconds" << std::endl;

	// store the model into file
	tdl.save("weight_20000final.bin");
